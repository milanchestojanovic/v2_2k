...
Int_t Low, High;
  ...
  TH1D *h_eta;
  TH1D *h_Mult;
  TH1D *h_M0; TH1D *h_M1; TH1D *h_M2; TH1D *h_M3; TH1D *h_M4; TH1D *h_M5; TH1D *h_M6; TH1D *h_M7; TH1D *h_M8; TH1D *h_M9; TH1D *h_M10; TH1D *h_M11; TH1D *h_M12; TH1D *h_M13; TH1D *h_M14; TH1D *h_M15;
...
//   TrkEff2018PbPb trkEff =  TrkEff2018PbPb("general", false, "");
  ...
  Double_t HFsumET;
   ...
   if( useCentrality_ )
   {
     ...
     occ = 200 - h_hiHF->FindBin(HFsumET);
   //  cout<<occ <<" " <<cbin<<endl;
     if ( occ >= 160 ) return;
   }
...
//        ++itTrack) {
	...
	if (pT>3.0) continue;
        ...
        // eff. Up 5%
	//eff_ref[int(Ntr)] = 1.0/(trkEff.getCorrection(itTrack.pt(), itTrack.eta(), occ) + 0.05);
	...
	l = occ/2/5;
        ...
        if (l==0) h_M0->Fill(Ntrk);
        if (l==1) h_M1->Fill(Ntrk);
        if (l==2) h_M2->Fill(Ntrk);
        if (l==3) h_M3->Fill(Ntrk);
        if (l==4) h_M4->Fill(Ntrk);
        if (l==5) h_M5->Fill(Ntrk);
        if (l==6) h_M6->Fill(Ntrk);
        if (l==7) h_M7->Fill(Ntrk);
        if (l==8) h_M8->Fill(Ntrk);
        if (l==9) h_M9->Fill(Ntrk);
        if (l==10) h_M10->Fill(Ntrk);
        if (l==11) h_M11->Fill(Ntrk);
        if (l==12) h_M12->Fill(Ntrk);
        if (l==13) h_M13->Fill(Ntrk);
        if (l==14) h_M14->Fill(Ntrk);
        if (l==15) h_M15->Fill(Ntrk);
...
Qnana::initHistos(const edm::Service<TFileService> & fs)
{
  ...
  //VertexCompositeNtuple = fs->make< TTree>("VertexCompositeNtuple","VertexCompositeNtuple");
   ...
   //VertexCompositeNtuple->Branch("Ntrk",&Ntrk,"Ntrk/I");
   //VertexCompositeNtuple->Branch("phi",phi_ref,"phi_ref[Ntrk]/D");
   //VertexCompositeNtuple->Branch("eff",eff_ref,"eff_ref[Ntrk]/D");
  ...
  //h_hiHF = fs->make<TH1D>("hiHF edges"," ",200,hiHFbinedgesSysDown);
  //h_hiHF = fs->make<TH1D>("hiHF edges"," ",200,hiHFbinedgesSysUp);
  ...
  h_pT  = fs->make<TH1D>("pT" , "" , 1000 , 0, 100 );
  h_eta  = fs->make<TH1D>("eta" , "" , 96 , -2.4, 2.4 );
  h_Mult  = fs->make<TH1D>("Mult" , "" , 300 , 0, 3000 );
  
  h_M0  = fs->make<TH1D>("Mult_0" , "" , 400 , 0, 4000 );
  h_M1  = fs->make<TH1D>("Mult_1" , "" , 375 , 0, 3750 );
  h_M2  = fs->make<TH1D>("Mult_2" , "" , 350 , 0, 3500 );
  h_M3  = fs->make<TH1D>("Mult_3" , "" , 325 , 0, 3250 );
  h_M4  = fs->make<TH1D>("Mult_4" , "" , 300 , 0, 3000 );
  h_M5  = fs->make<TH1D>("Mult_5" , "" , 275 , 0, 2750 );
  h_M6  = fs->make<TH1D>("Mult_6" , "" , 250 , 0, 2500 );
  h_M7  = fs->make<TH1D>("Mult_7" , "" , 225 , 0, 2250 );
  h_M8  = fs->make<TH1D>("Mult_8" , "" , 200 , 0, 2000 );
  h_M9  = fs->make<TH1D>("Mult_9" , "" , 175 , 0, 1750 );
  h_M10  = fs->make<TH1D>("Mult_10" , "" , 150 , 0, 1500 );
  h_M11  = fs->make<TH1D>("Mult_11" , "" , 125 , 0, 1250 );
  h_M12  = fs->make<TH1D>("Mult_12" , "" , 100 , 0, 1000 );
  h_M13  = fs->make<TH1D>("Mult_13" , "" , 75 , 0, 750 );
  h_M14  = fs->make<TH1D>("Mult_14" , "" , 50 , 0, 500 );
  h_M15  = fs->make<TH1D>("Mult_15" , "" , 25 , 0, 250 );
